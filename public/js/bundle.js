...
var displayMap = function displayMap(locations) {
  ...
  var map = new mapboxgl.Map({
    ...
    scrollZoom: false // center: [ -118.214010,33.981452],
    // zoom: 10,
    // interactive: false
  ...
  locations.forEach(function (loc) {
    ...
    el.className = 'marker'; // 2) Add marker

    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); // 4)) Add popup

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // 3) Extend map bounds to include current location
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 7:
            ...
            (0, _alerts.showAlert)('error', 'Error log out. Try again later!');
...
var stripe = Stripe('pk_test_51H83D9CGOvvjwdDpMkJ1oPaO9CRx51qTEYe7g9DOTyxrEgyw82YAKTqAVRzQNmOXuceaF5TBaiTF6VmE4MxxxqNX00XaejWHbm'); // this tourId parameter will take tourId comes from the webpage through data attribute. This function will be called in index.js with an event listener in next commit
...
if (bookBtn) bookBtn.addEventListener('click', function (e) {
  e.target.textContent = 'Processing...'; // in java script the tour-id from webpage is written as tourId(camelcase). it is automatically converted.
